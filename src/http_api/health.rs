...
async fn live() -> impl IntoResponse {
    (
        StatusCode::OK,
        axum::Json(
            json!({"ok": true, "pid": std::process::id(), "uptime_seconds": crate::ops::runtime_snapshot().uptime_seconds}),
        ),
    )
}
...
async fn ready(State(st): State<HttpState>) -> impl IntoResponse {
    ...
    (
        status,
        axum::Json(json!({"ok": ok, "phase": runtime.startup_phase, "store_ok": store_ok})),
    )
}
    ...
    match st.store.stats() {
        Ok((docs, chunks, nodes, edges)) => {
            ...
            let relational_integrity_ok =
                orphan_chunks == 0 && orphan_nodes == 0 && orphan_edges == 0;
