...
use crate::ingest::{
    IngestCommand, IngestFileCommand, IngestService, ReembedDocumentResult,
    UpdateDocumentCommand, UpdateDocumentResult,
};
...
impl RagServer {
    ...
    async fn doctor_repair(
        ...
        let report = DiagnosticsService::new(&self.store, &self.config)
            .repair(&self.embedder, params.dry_run.unwrap_or(true), params.max_docs)
            .await
            .map_err(Self::map_err)?;
        Self::json_result(&report)
    }
...
pub(crate) fn recovery_path(raw: &str, roots: &[PathBuf]) -> Result<PathBuf, AppError> {
    let raw = raw.trim();
    if raw.is_empty() {
...
pub(crate) fn refuse_live_db_target(path: &Path, db_path: &Path) -> Result<(), AppError> {
...
impl ServerHandler for RagServer {
    ...
    async fn call_tool(
        ...
        let args = request.arguments.clone().map(serde_json::Value::Object);
        let call = crate::telemetry::begin(&agent, self.transport, &name, args.as_ref());
        let tcc = ToolCallContext::new(self, request, context);
        let result = self.tool_router.call(tcc).await;
        match &result {
            Ok(value) => call.finish(!value.is_error.unwrap_or(false), None, result_hint(value)),
            Err(error) => call.finish(false, Some(error.to_string()), None),
        }
        result
    }
}
...
fn result_hint(result: &CallToolResult) -> Option<String> {
    ...
    match value {
        ...
        serde_json::Value::Object(map) => ["count", "chunk_count", "hits", "total"]
            .iter()
            .find_map(|key| map.get(*key).and_then(|v| v.as_u64()).map(|n| format!("{key} {n}")))
            .or_else(|| map.get("op").and_then(|v| v.as_str()).map(str::to_string)),
        _ => None,
    }
}
