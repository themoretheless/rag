...
| `link_nodes` | Explicit `origin=explicit` edge; `rel_type` from the §1.1 vocabulary (structural names or a wiki semantic one such as `реализует`) | Manual cross-ref | `mempalace_create_tunnel` (cross-wing) | v1 |
