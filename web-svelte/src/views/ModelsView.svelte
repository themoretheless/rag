<script lang="ts">
  ...
  import { api, hasGatewayToken, loadPanels, setGatewayToken } from '@/api/client'
  ...
  const tokenStored = hasGatewayToken()
  ...
  function updateToken(value: string) {
    ...
    try {
      ...
      tokenDraft = ''
      // Drop all cached data and restart loaders with the new credentials.
      window.location.reload()
    } catch (cause) {
      tokenError = cause instanceof Error ? cause.message : 'Не удалось обновить токен.'
    }
  }
...
<div class="models-page screen">
  ...
  <form class="connection panel" onsubmit={(event) => { event.preventDefault(); updateToken(tokenDraft) }}>
    <div><strong>Доступ к серверу</strong><p>{tokenStored ? 'Токен сохранён для этой вкладки.' : 'Если сервер требует авторизацию, введите токен доступа.'} После изменения страница перезагрузится.</p></div>
    <label for="gateway-token">Токен доступа<input id="gateway-token" name="gateway-token" type="password" autocomplete="off" autocapitalize="off" spellcheck={false} bind:value={tokenDraft} placeholder={tokenStored ? 'Новый токен' : 'Bearer token'} aria-describedby="gateway-token-help" /></label>
    <div class="connection-actions"><button class="primary" type="submit" disabled={!tokenDraft.trim()}>{tokenStored ? 'Заменить токен' : 'Подключиться'}</button><button class="secondary" type="button" disabled={!tokenStored} onclick={() => updateToken('')}>Удалить токен</button></div>
    <p id="gateway-token-help" class="connection-help">Токен хранится только в сессии этой вкладки и отправляется серверу в заголовке запроса.</p>
    {#if tokenError}<p role="alert" class="connection-error">{tokenError}</p>{/if}
  </form>
